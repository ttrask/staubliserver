...
public:	
	...
	void SetFrame( SFrame *pFrame )	{ m_pFrame = pFrame; }
	void SetInput( float *pfInput )	{ m_pfInput = pfInput; }
	void SetMatrix( const D3DXMATRIX &mInitial )	{ m_Body.SetMatrix( mInitial ); }

	void Tick( float fElapsedTime )
	{
		// simulate
		m_Body.Simulate( fElapsedTime );

		// store result into frame
		m_Body.GetMatrix( m_pFrame->matRot );
...
public:
  ...
  HRESULT TickControllers(float fElapsedTime);
  ...
  D3DMATRIX *GetTranslationMatrix();
	...
	Controller *AttachPhysicsController( const char *szBoneName, float *fInput );
	
  SDrawElement* GetHeadDrawElement() { return m_pdeHead; }
  const char* GetPathname() { return m_szFileName; }
