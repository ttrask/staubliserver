...
HRESULT C3DEngine::Render()
{
  ...
  if( m_bActive && m_bReady )
  {
    ...
    //  look at point is fixed for now
//    D3DXVECTOR3& vLookatPt0 = m_pCamera->GetLookatPt();
//    D3DXVECTOR3& vEyePt0 = m_pCamera->GetEyePt();
//    D3DXVECTOR3& vUp0 = m_pCamera->GetUpVec();

    D3DXVECTOR3& vLookatPt0 = m_pCamera->GetLookatPt();

    D3DXVECTOR3 vLookatPt = m_pCamera->GetLookatPt();
    D3DXVECTOR4 vLookatPt4;
    D3DXVec3Transform( &vLookatPt4, &vLookatPt, m_ArcBall.GetTranslationDeltaMatrix() );
    vLookatPt = D3DXVECTOR3( vLookatPt4.x, vLookatPt4.y, vLookatPt4.z );

    m_ArcBall.ClearDeltaMatrices();
 
    // set eye point by rotating initial eye direction by arcball rotation
    const D3DXVECTOR3 vEyeDir = D3DXVECTOR3( 0.0f, 0.0f, -1.0f );
    D3DXVECTOR4 vEyePt4;
...
//    m_pFont->DrawText( 2, 20, D3DCOLOR_ARGB(255,255,255,0), m_strDeviceStats );
	...
	m_pFont->DrawText( 2,  0, D3DCOLOR_ARGB(255,255,255,0), m_strFrameStats );
	m_pFont->DrawText( 500,  440, D3DCOLOR_ARGB(70,70,255,120), "P5 Team" );
...
/****************/
// vecDir = 
//		D3DXVECTOR3(cosf(timeGetTime()/360.0f), 0.0f, sinf(timeGetTime()/360.0f) );
...
//	light.Direction = vecDir;
  ...
  if (FAILED(hr))
	...
	m_pd3dDevice->SetRenderState(D3DRS_AMBIENT, 0x00ff00ff);
...
HRESULT C3DEngine::SetProjectionMatrix()
{
  ...
  D3DXMatrixPerspectiveFovRH(&mat, 0.25f*3.141592654f, fAspect, 1.0f / 64, 1.0f * 200);
...
HRESULT C3DEngine::OneTimeSceneInit()
{
  ...
  m_pCamera->SetViewParams( D3DXVECTOR3(0.0f, 0.0f, 0.0f),  D3DXVECTOR3(0.0f, 0.0f, 0.0f),	D3DXVECTOR3(0.0f, 0.0f, 0.0f) );
  FLOAT fAspect = (float)Width() / (float)Height();
  m_pCamera->SetProjParams( 0.25f*D3DX_PI, fAspect, 1.0f / 64, 1.0f * 200);
...
HRESULT C3DEngine::TickControllers()
{
	...
	while (pChild)
	{
		HRESULT hr = pChild->TickControllers();
		if( FAILED( hr ) )
			return hr;
		pChild = m_pChildList->IteratorNext();
  }
